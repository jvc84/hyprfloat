...
//     }
// };
// use crate::{get_table, notify_error, update_data, CONFIG_FILE, CONFIG_DATA, CLIENT_DATA, EXACT, MONITOR_DATA, get_daemon_data, LAYERS_DATA, GIVE_ARGS};
// use clap::Parser;
//
// #[derive(Parser, Debug)]
...
//     let output = std::process::Command::new("/home/adex/.scripts/hypr/read_tags.sh")
//         .output()
...
//
//
// fn tag_window(axis: &str, tag: &str) -> Result<(), Box<dyn std::error::Error>> {
...
//
//     let class = cli.class;
//     let table = get_table("windows", CONFIG_FILE.read().unwrap().clone().as_str());
...
//
//     let parsed_args = Args::parse_from(
//         GIVE_ARGS.read().unwrap().clone()
//     );
...
//     }
//
//     // let _ = get_daemon_data(parsed_args.config);
//     println!("{:#?}", CONFIG_DATA.read().unwrap().clone());
//     update_data();
//
